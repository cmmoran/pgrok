...
func (t *Tunnel) listenTcp(listener *net.TCPListener) {
	for {
		...
		connection := conn.Wrap(tcpConn, "pub")
		connection.AddLogPrefix(t.Id())
		connection.Info("New connection from %v", connection.RemoteAddr())

		go t.HandlePublicConnection(connection)
	}
}
...
func (t *Tunnel) HandlePublicConnection(publicConn conn.Conn) {
	...
	var rreplacements = make(map[string]string)
	if t.req.HostHeader != "" {
		ohost := strings.Replace(t.url, t.req.Protocol+"://", "", 1)
		replacements["Host: "+ohost] = "Host: " + t.req.HostHeader
		rreplacements[t.req.HostHeader] = ohost
	}
	for find, replace := range replacements {
		publicConn.Info("Find/Replace for Join [%s] -> [%s]", find, replace)
	}

	// join the public and proxy connections
	bytesIn, bytesOut := conn.JoinWithReplace(replacements, rreplacements, publicConn, proxyConn, true, true)
