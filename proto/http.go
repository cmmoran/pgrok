...
func (h *Http) WrapConn(c conn.Conn, hostHeader string, ctx interface{}) conn.Conn {
	tee := conn.NewTee(c)
	lastTxn := make(chan *HttpTxn)
	go h.readRequests(tee, hostHeader, lastTxn, ctx)
	go h.readResponses(tee, lastTxn)
	return tee
}

func (h *Http) readRequests(tee *conn.Tee, hostHeader string, lastTxn chan *HttpTxn, connCtx interface{}) {
	defer close(lastTxn)
	...
	for {
		...
		if hostHeader != "" {
			tee.Info("Fixing Headers: %s -> %s", req.Header.Get("Host"), hostHeader)
			req.URL.Host = hostHeader
			req.Header.Set("Host", hostHeader)
			if req.Header.Get("Referer") != "" {
				req.Header.Set("Referer", req.URL.Scheme+"://"+hostHeader+"/")
			}
			if req.Header.Get("Origin") != "" {
				req.Header.Set("Origin", req.URL.Scheme+"://"+hostHeader)
			}
		} else {
			req.URL.Host = req.Host
		}
