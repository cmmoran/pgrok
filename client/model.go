...
func (c *ClientModel) PlayRequest(tunnel mvc.Tunnel, payload []byte) {
	...
	localConn = tunnel.Protocol.WrapConn(localConn, c.tunnelConfig[tunnel.PublicUrl].HostHeader, mvc.ConnectionContext{Tunnel: tunnel, ClientAddr: "127.0.0.1"})
...
func (c *ClientModel) control() {
	...
	for {
		...
		case *msg.NewTunnel:
			...
			c.tunnelConfig[tunnel.PublicUrl] = reqIdToTunnelConfig[m.ReqId]
...
%s`, len(badGatewayBody), badGatewayBody)))
	...
	m.connTimer.Time(func() {
		localConn.Info("Connecting to startPxy.ClientAddr: %s", startPxy.ClientAddr)
		localConn := tunnel.Protocol.WrapConn(localConn, c.tunnelConfig[startPxy.Url].HostHeader, mvc.ConnectionContext{Tunnel: tunnel, ClientAddr: startPxy.ClientAddr})

		bytesIn, bytesOut := conn.Join(localConn, remoteConn)
		localConn.Debug("Bytes In: [%d : %d] -> [%d : %d]", bytesIn, m.bytesInCount.Count(), bytesOut, m.bytesOutCount.Count())
